...
var ematPorEnviarOpciones = <?php echo json_encode($_GLOBALS['SafiRequestVars']['opciones'],JSON_FORCE_OBJECT); ?>;
</script>
<script type="text/javascript">
$().ready(function(){
	
$("#pedrito").show("drop",{ direction: "left" },300);


$("#bentrada").click(function() {

$("#pedrito").hide("drop",{ direction: "left" },300,function(){

	window.location = "disminuirMatBandeja.php";
...
$("#RevisionesYMemosEmat").click(function() {
	
    
	if($("#RevisionesYMemosEmat").attr('opcion') == 0 ){

		$("#RevisionesYMemosEmat").html('.:Detalle emat :.');
		$("#RevisionesYMemosEmat").attr('opcion',1);
		
		$("#window9").hide();
    	$("#window10").show('blind',300);
		...
		$("#RevisionesYMemosEmat").html('.:Revisiones y memos :.');
		$("#RevisionesYMemosEmat").attr('opcion',0);
		
		$("#window10").hide();
    	$("#window9").show('blind',300);
...
function SearchDetalleEmat(id,opcion,idcadenaactual){

	$.ajax({
	url: "../../../recursos/almacen/inventario/materiales.php",
		Type: "post",
		dataType: "json",
		data: {
			accion: "SearchEmatDetalle",
			tipoRespuesta: 'json',
			key: id,
		...
		success: function(json){
			...
			if(observacionesDoc = json.observacionesDoc){
				...
				$('#noDocumentosAsociadosEmat').hide();
				$('#tablaDocumentosAsociadosEmat').children('tr').remove();
				num = 1;
			...
			//		alert(val.perfilNombre+" / "+val.observacion+" / "+cadena[2]+"-"+cadena[1]+"-"+cadena[0]);
			    ...
			    var tbody = $('#tablaDocumentosAsociadosEmat')[0];	
			  ...
			  //  alert(val.cargoDependencia);	
				...
				$('#tablaDocumentosAsociadosEmat').children('tr').remove();
				$('#noDocumentosAsociadosEmat').show();
			...
			if(revisiones = json.revicionesDoc){
					...
					$('#noRegistrosRevisionEmat').hide();
					$('#tablaRevisionEmat').children('tr').remove();
					num = 1;
				  ...
				  //  alert(val.cargoDependencia);	
					...
					$('#tablaRevisionEmat').children('tr').remove();
					$('#noRegistrosRevisionEmat').show();
					
					
				}	


			 $('span[detalle=\'emat\']').html(id);

			$('td[detalle=\'proveedoremat\']').html(json.proveedor);

			$('td[detalle=\'ubicacionemat\']').html(json.ubicacion);

			$('td[detalle=\'dependenciaemat\']').html(json.depe_solicitante[0]);

			//alert(JSON.stringify(json.fecha));
			
			 $('#ematTbody').find('.trCaso').remove();
			
			var tbody = $('#ematTbody')[0];
			...
			//alert(JSON.stringify(val)+" serial:"+json.serial[id]);
						...
						fila.className='normalNegro trCaso';
				 		...
				 		columna2.setAttribute("valign","top");
				 		columna2.setAttribute("style","font-size:10px");
				 		columna2.appendChild(document.createTextNode(json.idarticulo[id]));
						...
						columna9.appendChild(document.createTextNode(json.alm_fecha_recepcion[id]));
...
$('.opcionesEmatcerrar').remove();

if(opcion == 'ematPorEnviarOpciones'){

if(ematPorEnviarOpciones){

	 $.each(ematPorEnviarOpciones[idcadenaactual], function(index, value) {
		 

		          var fila = $('#trOpcionesDetallesEmat')[0];	
				  var columna = document.createElement("td");
				    columna.setAttribute("class","opcionesEmatcerrar");
                 ...
                 alink.setAttribute("onclick",'AccionesEmat(this)');
	  ...
	  $('#OpcionesPdfEmat').hover(function(){
		...
		$('#OpcionesPdfEmat').click(function(event) {
			
			 url = "entradas_pdf.php?id="+$('span[detalle=\'emat\']').html()+"";
	  	   // window.location = url;
...
if(obj.attr('id') == 'Modificar'){
  ...
  url = "../../../recursos/almacen/inventario/modificar_materiales.php?emat="+$('span[detalle=\'emat\']').html()+"";
  window.location = url; 

}else{

 if(confirm("\u00BFEst\u00E1 seguro que desea ("+obj.attr('id')+") esta 'emat'?")){

 if(obj.attr('id') == 'Anular' || obj.attr('id') == 'Devolver'){

 var lugar = 'Entrada de Materiales';
...
if(memo){
 ...
 url = "../../../recursos/almacen/inventario/materiales.php?accion=ProcesarEmat&opcion=0&memo="+memo+"&emat="+$('span[detalle=\'emat\']').html()+"&idCadenaSigiente="+obj.attr('idCadenaSigiente')+"&idopcion="+obj.attr('idopcion')+"&accRealizar="+obj.attr('id')+"";
}
 
 }else{
	 
	 
     url = "../../../recursos/almacen/inventario/materiales.php?accion=ProcesarEmat&emat="+$('span[detalle=\'emat\']').html()+"&idCadenaSigiente="+obj.attr('idCadenaSigiente')+"&idopcion="+obj.attr('idopcion')+"&accRealizar="+obj.attr('id')+"";	   
...
?>
	<table align="right">
			...
			<a href="javascript:void(0);" id="bentrada" >.:Bandeja de Disminuci&oacute;n:.</a>
			</td>
		</tr>
	</table>
<?php 
...
?>
	<table style="width: 100%;">
		<tr>
			<td style="text-align: center;" class="normalNegroNegrita"><span
				style="padding-bottom: 20px; display: block;">Documentos de Registro por Enviar/Aprobar/Finalizar</span>
			</td>
		</tr>
		<tr>
			<td><table cellpadding="0" cellspacing="0" align="center"
					...
					<tr>
						...
						<th width="54%" class="header"><span class="normalNegroNegrita">Proveedor</span>
						</th>
					...
					<tr onclick="Registroclikeado(this);"
						...
						<td><a href="#dialog" docgId="<?php echo $index['docg_id']; ?>"
							class="detalleOpcion" opcion="null"  tipoDetalle="materiales"> <?php echo $index['docg_id'] ?>
						</a>
						</td>
						...
						from
							sai_arti_inco t1
							inner join sai_proveedor_nuevo t2 on(t2.prov_id_rif = t1.proveedor)
  						where
  							acta_id='".$index['docg_id']."'
  						";
						$resul=pg_exec ( $conexion, $queryobser );
						$row = pg_fetch_array ( $resul );
						$obser=$row['prov_nombre'];
						echo $obser;
						?></td>
					...
					var pctaPorEnviarOpciones = <?php echo json_encode($GLOBALS['SafiRequestVars']['opciones'],JSON_FORCE_OBJECT); ?>;
							...
							class="detalleOpcion" opcion="ematPorEnviarOpciones"
							docgId="<?php echo $index['docg_id']; ?> "
							idCadenaActual="<?php echo $index['wfca_id'] ?>" tipoDetalle="materiales">Seleccionar</a>
						</td>
					</tr>
	...
	<table style="width: 100%;">
		...
		<tr>
			<td style="text-align: center;" class="normalNegroNegrita"><span
				style="padding-bottom: 20px; display: block;">Documentos de Registro Devueltos</span>
			</td>
		</tr>
	<tr>
			<td><table cellpadding="0" cellspacing="0" align="center"
	...
	<table style="width: 100%;">
		...
		<tr>
			<td style="text-align: center;" class="normalNegroNegrita"><span
				style="padding-bottom: 20px; display: block;">Documentos Registro en
		...
		<tr>
			<td><table cellpadding="0" cellspacing="0" align="center"
					...
					<tr>
						...
						<th width="15%"  class="header"><span class="normalNegroNegrita">Fecha</span>
						<th width="40%"  class="header"><span class="normalNegroNegrita">Proveedor</span>
						</th>
