...
<tr>
  ...
  <select name="cod_bien" id="cod_bien" class="normalNegro" disabled="true">
	   ...
	   <?php
	    $sqlc="SELECT * FROM sai_seleccionar_campo('sai_arti_tipo','tp_id','id_tipo=4','tp_id',1) resultado_set(tp_id varchar)"; 
...
<tr>
  ...
  <select name="des_bien" id="des_bien" class="normalNegro" disabled="true">
	   <option value="0">[Seleccione]</option>
	   <?php
	    $sql_d="SELECT * FROM sai_seleccionar_campo('sai_arti_tipo','tp_id,tp_desc','id_tipo=4','tp_desc',1) resultado_set(tp_id varchar, tp_desc varchar)"; 
...
elseif ($_POST['hid_buscar1']==2)
   { 	
	 	   if (($_POST['cod_bien'])!=0)  
	 	   	$codigo=trim($_POST['cod_bien']); 
		   else 
		    $codigo=trim($_POST['des_bien']); 
		    $sql_tab="SELECT tp_desc FROM sai_arti_tipo WHERE tp_id='".$codigo."'";
