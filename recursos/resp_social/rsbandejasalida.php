...
<script type="text/javascript">
var nePorEnviarOpciones = <?php echo json_encode($_GLOBALS['SafiRequestVars']['opciones'],JSON_FORCE_OBJECT); ?>;
</script>
<script type="text/javascript">
$().ready(function(){
...
$("#pedrito").hide("drop",{ direction: "right" },300,function(){
	...
	window.location = "respsocialBandeja.php";
...
$("#RevisionesYMemosNe").click(function() {
	
    
	if($("#RevisionesYMemosNe").attr('opcion') == 0 ){

		$("#RevisionesYMemosNe").html('.:Detalle ne :.');
		$("#RevisionesYMemosNe").attr('opcion',1);
		
		$("#window7").hide();
    	$("#window8").show('blind',300);
		...
		$("#RevisionesYMemosNe").html('.:Revisiones y memos :.');
		$("#RevisionesYMemosNe").attr('opcion',0);
		
		$("#window8").hide();
    	$("#window7").show('blind',300);
...
function SearchDetalleNe(id,opcion,idcadenaactual){

			$.ajax({
			url: "../../recursos/resp_social/acciones/respsocialsal.php",
				Type: "post",
				dataType: "json",
				data: {
					accion: "SearchNeDetalle",
					tipoRespuesta: 'json',
					key: id,
				...
				success: function(json){
					...
					if(observacionesDoc = json.observacionesDoc){
						...
						$('#noDocumentosAsociadosNe').hide();
						$('#tablaDocumentosAsociadosNe').children('tr').remove();
						num = 1;
				...
				tdClass = (tdClass == "even") ? "odd" : "even";
					    var tbody = $('#tablaDocumentosAsociadosNe')[0];	
					...
					if(revisiones = json.revicionesDoc){
							...
							$('#noRegistrosRevisionNe').hide();
							$('#tablaRevisionNe').children('tr').remove();
							num = 1;
						  ...
						  //  alert(val.cargoDependencia);	
							...
							$('#tablaRevisionNe').children('tr').remove();
							$('#noRegistrosRevisionNe').show();
							
							
						}	


					 $('span[detalle=\'ne\']').html(id);

					$('td[detalle=\'rsfechasal\']').html(json.fecha_recepcion);

					$('td[detalle=\'rsobservacionsal\']').html(json.destino);

					//alert(JSON.stringify(json.fecha));
					
					 $('#neTbody').find('.trCaso').remove();
...
$('.opcionesNecerrar').remove();

	if(opcion == 'nePorEnviarOpciones'){

		if(nePorEnviarOpciones){

	    	 $.each(nePorEnviarOpciones[idcadenaactual], function(index, value) {
	    		 
	   
	    		          var fila = $('#trOpcionesDetallesNe')[0];	
						  var columna = document.createElement("td");
						    columna.setAttribute("class","opcionesNecerrar");
                         ...
                         alink.setAttribute("onclick",'AccionesNe(this)');
 		  ...
 		  $('#OpcionesPdfNe').hover(function(){
 			...
 			$('#OpcionesPdfNe').click(function(event) {
 				
 				 url = "nota_entrega_pdf.php?id="+$('span[detalle=\'ne\']').html()+"";
 		  	   // window.location = url;
...
function AccionesNe(obj){
     ...
     //alert(obj.attr('id'));
	  ...
	  if(obj.attr('id') == 'Modificar'){
		  ...
		  url = "../../recursos/resp_social/modificar_respsocialsal.php?ne="+$('span[detalle=\'ne\']').html()+"";
		  window.location = url; 
      
    }else{
        
    	 if(confirm("\u00BFEst\u00E1 seguro que desea ("+obj.attr('id')+") esta 'ne'?")){
    	...
    	if(memo){
         ...
         url = "../../recursos/resp_social/acciones/respsocialsal.php?accion=ProcesarNe&opcion=0&memo="+memo+"&ne="+$('span[detalle=\'ne\']').html()+"&idCadenaSigiente="+obj.attr('idCadenaSigiente')+"&idopcion="+obj.attr('idopcion')+"&accRealizar="+obj.attr('id')+"";
    	}
         
    	 }else{
    		 
    		 
	         url = "../../recursos/resp_social/acciones/respsocialsal.php?accion=ProcesarNe&ne="+$('span[detalle=\'ne\']').html()+"&idCadenaSigiente="+obj.attr('idCadenaSigiente')+"&idopcion="+obj.attr('idopcion')+"&accRealizar="+obj.attr('id')+"";	   
...
<div id="pedrito" style="display: none;">
	<table>
			...
			<a href="javascript:void(0);" id="bentrada" >.:Bandeja de Entrada:.</a>
			</td>
		</tr>
	</table>
		...
		<tr>
			<td><table cellpadding="0" cellspacing="0" align="center"
					...
					<tr>
						...
						<th width="54%" class="header"><span class="normalNegroNegrita">Detalle del destino</span>
						</th>
					...
					<tr onclick="Registroclikeado(this);"
						...
						<td><a href="#dialog" docgId="<?php echo $index['docg_id']; ?>"
							class="detalleOpcion" opcion="null"  tipoDetalle="respsocialsal"> <?php echo $index['docg_id'] ?>
						</a>
						</td>
					...
					var pctaPorEnviarOpciones = <?php echo json_encode($GLOBALS['SafiRequestVars']['opciones'],JSON_FORCE_OBJECT); ?>;
							...
							class="detalleOpcion" opcion="nePorEnviarOpciones"
							docgId="<?php echo $index['docg_id']; ?> "
							idCadenaActual="<?php echo $index['wfca_id'] ?>" tipoDetalle="respsocialsal">Seleccionar</a>
						</td>
					</tr>
		...
		<tr>
			<td><table cellpadding="0" cellspacing="0" align="center"
					...
					<tr>
						...
						<th width="15%"  class="header"><span class="normalNegroNegrita">Fecha</span>
						<th width="40%"  class="header"><span class="normalNegroNegrita">Detalle de destino</span>
						</th>
