...
if($error==""){
	if($tipo==TIPO_IMPUTACION_PROYECTO){
		...
		if($row[0]==0){
			$error = "8";//No existe un proyecto con este código y este año presupuestario.
		}
		...
		$row=pg_fetch_array($resultado);
		if($row[0]==0){
			$error = "9";//No existe una acción centralizada con este código y este año presupuestario.
		}
	}
}
if($error == ""){
	...
	$sql="SELECT * FROM sai_modificar_categoria_programatica(".
															"".$tipo.",".
															"".$anioPresupuestario.",".
															"'".$codigo."',".
															"'".$titulo."',".
	...
	header("Location:detalle.php?codigo=".$codigo."&anioPress=".$anioPresupuestario."&tipo=".$tipo."&accion=modificar",false);
}else{
	header("Location:modificar.php?msg=".$error."&codigo=".$codigo."&anioPress=".$anioPresupuestario."&tipo=".$tipo,false);
}
ob_end_flush();
pg_close($conexion);
?>