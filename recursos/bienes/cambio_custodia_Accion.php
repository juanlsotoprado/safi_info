...
<?php
  ...
  {	
   ...
   require_once("../../includes/arreglos_pg.php");
    ...
    $arreglo_activos= convierte_arreglo ($lista_mobiliarios);
	else
	$arreglo_activos="{}";
    ...
    $sql =  "Select * from sai_insert_bienes_custodia ('".$arreglo_id."','".$arreglo_cantidad."','";
	$sql .= $motivo."','".$_SESSION['login']."','".$arreglo_medida."', '".$_SESSION['user_depe_id']."','".$arreglo_activos."',$ubicacion) as ingresado ";
...
<form action="" name="form" id="form" method="post" >
 ...
 <?php
  ...
  {	
  	$query_custodia="SELECT cantidad FROM sai_bien_item_custodia WHERE id='".$id[$i]."' and acta_id='".$codigo."'";
   ...
   <tr>
	 <td class="normal" colspan="2"><strong>Activos</strong></td>
	</tr>
	 <?php
  	 for ($i=0; $i< $cantidad_activos;$i++)
     {	?>
   <tr>
	  <td class="normal" colspan="2"><?php echo $lista_mobiliarios[$i];?></td>
   </tr>
	  <? 
   }?>
   	<tr>
  ...
  <br><br>
  <div align='center'>
  <a target="_blank" class="normal" href="custodia_pdf.php?codigo=<?=$codigo;?>" class="link">
