...
if ($rev=pg_fetch_array($result_rev)){
 ...
 $sql_salida="SELECT asbi_destino, to_char(asbi_fecha,'DD/MM/YYYY') as fecha,infocentro,solicitante,empl_nombres,empl_apellidos,t1.esta_id
 FROM sai_bien_asbi t1,sai_empleado t3 
 WHERE t1.asbi_id='".$codigo."' and empl_cedula=t1.usua_login";
...
$contenido = "<style type='text/css'>
				...
				<tr><td ><td align='right' class='textoTabla'><b>N&deg; Acta: </b>".$new_codigo."<br></td></tr>
				<tr><td ><td align='right' class='textoTabla'><b>Elaborado por: </b>".$elaborado."</b><br></td></tr>
				</table>
				<br><br>";										  
				if($tipo=='s')
$contenido .=	"<div align='center' class='textoTabla'><b>AUTORIZACI&Oacute;N DE SALIDA DE ACTIVOS Y/O MATERIALES</b></div><br>";
				else 
...
$contenido .="<table border=1 width='100%'>".
 ...
 $sql_detalle="SELECT count(t1.clave_bien) as cantidad,bien_id,nombre FROM sai_bien_asbi_item t1,sai_biin_items t2,sai_item t3
 WHERE asbi_id='".$codigo."' and t1.clave_bien=t2.clave_bien and cantidad=0 and t2.bien_id=t3.id
 ...
 { 
	...
	$sql_bien="SELECT * FROM sai_biin_items t1,sai_bien_marca,sai_bien_asbi_item t3 WHERE t1.clave_bien=t3.clave_bien and bmarc_id=marca_id and asbi_id='".$codigo."' and bien_id='".$rowd['bien_id']."'";
...
//Artículos
$sql_arti="SELECT t3.nombre,t2.cantidad,t2.medida FROM sai_bien_asbi t1,sai_bien_asbi_item t2,sai_item t3
WHERE t1.asbi_id='".$codigo."' and t1.asbi_id=t2.asbi_id and id=arti_id ORDER BY t3.nombre";
