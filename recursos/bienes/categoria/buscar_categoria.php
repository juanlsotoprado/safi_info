...
<tr class="td_gray"> 
      <td height="15" colspan="3" valign="midden" class="normalNegroNegrita">B&uacute;squeda de categor&iacute;as de activos</td>
</tr>
<tr>
  ...
  <select name="cod_bien" id="cod_bien" class="normalNegro" disabled="true">
	   ...
	   <?php
	    $sqlc="SELECT * FROM sai_seleccionar_campo('bien_categoria','id','','id',1) resultado_set(id int2)"; 
...
<tr>
  ...
  <select name="des_bien" id="des_bien" class="normalNegro" disabled="true">
	   <option value="0">[Seleccione]</option>
	   <?php
	    $sql_d="SELECT * FROM sai_seleccionar_campo('bien_categoria','id,nombre','','nombre',1) resultado_set(id int2, nombre varchar)"; 
	    $resultado_set_d=pg_query($conexion,$sql_d) or die("Error al mostrar");
	    while($rowd=pg_fetch_array($resultado_set_d)) 
	    { 
 		 $marc_id=$rowd['id'];
		 $marc_descripcion=$rowd['nombre'];
	   ?>
...
if ($_POST['hid_buscar1']==1)
{
...
else
   {
	   if ( (($_POST['cod_bien'])!=0) or (($_POST['des_bien'])!=0) )
	   {
		   ...
		   $sql_tab="SELECT * FROM sai_seleccionar_campo('bien_categoria','nombre','id=''$codigo''','',2)
		   resultado_set(nombre varchar)";  
		   ...
		   {   
				...
				<td align="center"><?=$codigo?></td>
    				<td align="center"><?=$rowa['nombre'];?></td>
