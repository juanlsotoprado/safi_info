...
$sql_marc = "select * from sai_seleccionar_campo('bien_categoria','nombre','id=''$categoria''','',0) Resultado_set(nombre varchar)";
...
function revisar()
{
	...
	palabra4=document.form1.marc_cod.value
   	...
   	<?php
         $sql_p="SELECT * FROM sai_seleccionar_campo('bien_categoria','nombre','','',2) resultado_set(nombre varchar)"; 

   	 $resultado_set_most_p=pg_query($conexion,$sql_p) or die("Error al mostrar");
   	 while($row=pg_fetch_array($resultado_set_most_p)) 
  	 {?>
		palabra3="<?php echo trim($row['nombre']); ?>"
		palabra5="<?php echo $row['id']; ?>"
		
		if ((palabra.toUpperCase()==palabra3.toUpperCase()) && (palabra4!=palabra5))
         {
    	 alert("Este categor\u00EDa ya se encuentra registrada");
        	}
        <?php
         }
  	?>
...
<table width="646" align="center" background="../../../imagenes/fondo_tabla.gif" class="tablaalertas">
	<tr class="td_gray"> 
	<td height="15" colspan="2" valign="midden" class="normalNegroNegrita"><strong>Modificar la categor&iacute;a de activos</strong></td>
    </tr>
	<tr>
	...
	<tr>
	  ...
	  <td height="38" valign="midden"><span class="peq_naranja">
		<input name="marc" type="text" class="normal" size="50" id="marc" value="<? echo($row_marc['nombre']); ?>" onkeyup="validar_digito(this);">
