...
{ 
   ...
   <?php
   $sql_p="SELECT * FROM sai_seleccionar_campo('sai_bien_marca','bmarc_nombre','','',2) resultado_set(bmarc_nombre varchar)"; 
   $resultado_set_most_p=pg_query($conexion,$sql_p) or die("Error al mostrar");
   while($row=pg_fetch_array($resultado_set_most_p)) 
   {?>
    palabra1="<?php echo trim($row['bmarc_nombre']); ?>"
	if (palabra.toUpperCase()==palabra1.toUpperCase())
    {
	 alert("Esta marca ya se encuentra registrada");
...
function revisar()
{
	...
	{
		alert("Debe escribir el nombre de la marca a registrar.")
...
<form name="form1" method="post" action="marca_e1.php">
<br />
<table width="466" align="center" background="../../../imagenes/fondo_tabla.gif" class="tablaalertas">
	<tr  class="td_gray"> 
	<td height="15" colspan="2" valign="midden" class="normalNegroNegrita"><strong>Registrar marca de activos</strong></td>
    </tr>
	<tr>
