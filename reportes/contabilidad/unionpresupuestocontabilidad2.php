...
$sql = " select ca.paga_id as caus_id, ca.paga_docu_id as caus_docu_id, case substr(ca.paga_docu_id,1,4) when 'codi' then ca.paga_docu_id when 'tran' then pt.docg_id
when 'pgch' then  pc.docg_id else '' end as docg_id,  substr(c.part_id,1,13) as part_id, cd.padt_monto as totalpres, 
 cd.padt_id_p_ac as proyecto, cd.padt_cod_aesp as especifica, c.part_id as part_id_completa,ca.esta_id as estatuspres, 
 ca.paga_fecha,  '' as fechatext,ca.fecha_anulacion,c.cpat_id ,dg.numero_reserva,s.comp_id, s.comp_fec,substring(s.comp_id,5),s.comp_fec_emis
...
((pt.docg_id=s.comp_doc_id and s.comp_id not like 'codi%') or 
  ...
  and s.esta_id<>15  and cd.part_id not like '4.11%' 
 
 and dg2.docg_id in (select docg_id from sai_pago_transferencia where trans_id in 
(select comp_doc_id from sai_comp_diario where comp_comen like 'T-%'  or comp_comen like 'P-%')
...
while($rowor=pg_fetch_array($resultado_set_most_or))
{
	...
	while($rowo=pg_fetch_array($resultado_set_most)) {		
	 ...
	 $fecha_anulacion = substr($rowor['fecha_anulacion'], 0, 4).substr($rowor['fecha_anulacion'], 5, 2).substr($rowor['fecha_anulacion'], 8, 2);
...
$tt=0;
			...
			if($rowor['estatuspres']=='15'){
					...
					to_date('".$rowor['fecha_anulacion']."', 'YYYY MM DD'),".$rowor['estatuspres'].",".$fecha_actual.",".$fecha_entrada.",'".$rowor['fechatext']."',".$vuelta.",'".$rowor['numero_reserva']."','".$caus_docu_id2."')";
								...
								$queryins="insert into sai_prescontableinicial2_".$login."  values('".$caus_id."','".$caus_docu_id."','".$part_id."',".$montopres*1.0.",'".$proyecto."','".$especifica."','".$comp_id."','".$cpat_id."',".$row['total']*1.0.",to_date('".$comp_fec."', 'YYYY MM DD'),
					null,".$rowor['estatuspres'].",".$fecha_actual.",".$fecha_entrada.",null,".$vuelta.",'".$rowor['numero_reserva']."','".$caus_docu_id2."')";	
							//}
...
from sai_prescontableinicial2_".$login." where  (comp_fec >= to_date('".$fechaIinicio."', 'DD MM YYYY') and comp_fec <= to_date('".$fechaFfin."', 'DD MM YYYY')) and fecha_anulacion is null

union

select  caus_id, caus_docu_id, part_id, monto_pres, proyecto, especifica, comp_id, cpat_id, monto_ctble, comp_fec, fecha_anulacion, esta_id, fechaactual,fechaentrada, fechaanulacion, posicion ,numero_reserva,doc2,posicion
from sai_prescontableinicial2_".$login." where  (fecha_anulacion >= to_date('".$fechaIinicio."', 'DD MM YYYY') and fecha_anulacion <= to_date('".$fechaFfin."', 'DD MM YYYY'))";
...
//TEMPORARY
$sql = " cREATE TEMPORARY TABLE sai_prescontable5_".$login." as 	(

SELECT part_id, $quer as posicion, sum(monto_pres) as monto_pres, proyecto, especifica,   sum(monto_ctble) as monto_ctble FROM sai_prescontable4_".$login." 
where ((comp_fec >= to_date('".$fechaIinicio."', 'DD MM YYYY') and comp_fec <= to_date('".$fechaFfin."', 'DD MM YYYY') and pos=1) or (fecha_anulacion >= to_date('".$fechaIinicio."', 'DD MM YYYY') and fecha_anulacion <= to_date('".$fechaFfin."', 'DD MM YYYY') and pos=2))
group by 1,2,4,5 order by 1,2)"; 
//echo $sql;
$resultado = pg_query($conexion,$sql) or die("Error al registrar saldos  en saiprescontable5_"); 

//echo $sql;
//$resultado = pg_query($conexion,$sql) or die("Error al registrar saldos iniciales en sai_banco_saldo2");
