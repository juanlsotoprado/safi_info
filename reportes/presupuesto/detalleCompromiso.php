...
/*$sql_str="SELECT * FROM sai_pres_reporte_mcompromisos_det(".$year_f.",'".$tipoImputacion."','".$idProyectoAccion."','".$idAccionEspecifica."','".$partida."','','".$opcionConsolidar."','".$year_i."/".$month_i."/".$day_i."','".$year_f."/".$month_f."/".$day_f."') as resultado";//
$res=pg_exec($sql_str);
...
if($opcionConsolidar == '0' && $centroGestor == ''){
	$query = 	"SELECT ".
					"scit.comp_id AS documento, scit.comp_monto AS monto ".
					",cast(substr(scit.comp_id,6) as integer) ".
				"FROM sai_comp_imputa_traza scit, ".
					"(".
					"SELECT ".
						"scit.comp_id, MAX(scit.comp_fecha) AS fecha ".
	...
	while($fila=pg_fetch_array($resultado)){
		$detalleCompromiso .= ','.$fila["documento"].':'.$fila["monto"];
	}
}else{
	$query = 	"SELECT ".
...
<?php
$compromisos = explode(",", $detalleCompromiso);
$sumatoria=0; 
for ($i=1;$i<count($compromisos);$i++) {
?>
	<tr class="normal">
	<?php
	list($codigo,$monto) = split( '[:]', $compromisos[$i]);
	$sumatoria+=$monto;
	?>
