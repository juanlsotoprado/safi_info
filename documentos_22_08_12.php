...
if ($request_id_tipo_documento=="sopg") {
	...
	left outer join sai_estado e on (trim(e.esta_id)=trim(d.esta_id))		
	where d.docg_id=pch.pgch_id and ch.docg_id=pch.docg_id and d.docg_id like '".$request_id_tipo_documento."%' AND d.esta_id=$estado_doc AND d.perf_id_act='".$user_perfil_id."' 
...
if ($request_id_tipo_documento=="sopg") {
	...
	left outer join sai_tipo_solicitud ts on (trim(s.sopg_tp_solicitud)=trim(ts.id_sol))			
	left outer join sai_estado e on (trim(e.esta_id)=trim(d.esta_id))		
	where d.docg_id=pch.pgch_id and ch.docg_id=pch.docg_id and d.docg_id like '".$request_id_tipo_documento."%' AND d.esta_id=$estado_doc_dev AND d.perf_id_act='".$user_perfil_id."' 
	ORDER BY s.sopg_fecha";
}
	else if ($request_id_tipo_documento=="tran") {
	$opcion_anular=1;	
...
?>
		...
		else if ($row_doc_transito['esta_id']==$estado_doc_dev && $user_perfil_id==$row_doc_transito['perf_id_act']) {	
			echo "<br>
			<a href='anular_documento.php?tipo=".$request_id_tipo_documento."&id=".trim($row_doc_transito['docg_id'])."' class='copyright'>Anular</a>	";
		}
	}
