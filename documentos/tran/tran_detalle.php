...
?>
<?php
$cod_doc = trim($_REQUEST['codigo']);
$codigo= $cod_doc;
$obs=trim($_POST['txt_observa']);

$sql="select s.sopg_monto as sopg_monto, d.depe_nombre as nombre_dependencia, ptr.trans_fecha as fecha_tran, ptr.nro_cuenta_emisor as numero_cuenta_emisor, ptr.docg_id as sopg, ptr.trans_asunto as asunto, ptr.trans_obs as observaciones, be.banc_nombre as nombre_banco_emisor,  upper(em.empl_nombres) || upper(em.empl_apellidos) as usuario_solicitante, em.empl_email as email, em.empl_tlf_ofic as telefono, ptr.nro_referencia as numero_referencia, ptr.trans_monto as monto_tran, upper(ptr.beneficiario) as beneficiario, t.nombre_sol as tipo_solicitud
...
if ($row=pg_fetch_array($resultado)) {
	...
	$nombre_banco_emisor = trim($row['nombre_banco_emisor'])."-".trim($row['numero_cuenta_emisor']);
...
$resultado=pg_query($conexion,$sql);
if ($row=pg_fetch_array($resultado)) {
	$nombre_banco_receptor = trim($row['nombre_banco_receptor'])."-".trim($row['numero_cuenta_receptor']);
}
...
<html>
<head>
<title>.:SAFI: Pago Transferencia</title>
	...
	<tr>
      ...
      <td colspan="2" class="normal" >
	   <table width="420" align="center">
		...
		<div align="center">
		<br><br>Este pago con transferencia fue consultado el d&iacute;a: 
