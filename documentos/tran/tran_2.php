...
if ($row=pg_fetch_array($resultado)) {
	...
	//echo "aaa".$numero_cuenta_receptor;
	...
	$nombre_banco_emisor = trim($row['nombre_banco']);
...
$sql="select ptr.nro_cuenta_receptor as numero_cuenta_receptor, br.banc_nombre as nombre_banco_receptor
from sai_pago_transferencia ptr, sai_banco br, sai_ctabanco ctbr
where ptr.nro_cuenta_receptor=ctbr.ctab_numero and ctbr.banc_id=br.banc_id and ptr.trans_id='".$codigo."'";
$resultado=pg_query($conexion,$sql);
if ($row=pg_fetch_array($resultado)) {
...
<html>
<head>
<title>.:SAFI: Revisar Pago con Transferencia</title>
...
function enviar() {   
	if(document.form.txt_concepto.value=="") {
		alert('Debe especificar el asunto del pago con transferencia');
...
//alert ("accion_ejecutar.php?tipo="+id_tipo_documento+"&accion="+id_objeto_actual+"&accion_sig=" + objeto_siguiente_id + "&hijo="+cadena_siguiente_id+"&opcion="+id_opcion+"&id="+id_documento);
	enviar();
}
...
<tr class="td_gray"> 
	...
	<tr>
		...
		<td class="normal"><input type="text" name="txt_monto_letras" id="txt_monto_letras" size="100" class="normal" disabled="disabled"></td>
	</tr>		
	<tr>
	...
	<tr>
		...
		<td class="normal"><input type="text" name="txt_referencia" id="txt_referencia"  value="<?php echo $numero_referencia;?>"></td>
	</tr>
	<tr>	
		...
		<select name="banco_emisor" class="normal" id="banco_emisor">
 		<option value="<?php echo $nombre_banco_emisor."-".$numero_cuenta_emisor;?>" selected="selected"><?php echo $nombre_banco_emisor;?>-<?php echo trim($numero_cuenta_emisor);?></option>			
 		  <?
			$sql="SELECT b.banc_id, upper(b.banc_nombre) as nombre_banco, c.ctab_numero as cuenta_banco from sai_banco b, sai_ctabanco c where b.banc_id=c.banc_id and c.ctab_estatus=1 and ctab_numero<>'".$numero_cuenta_emisor."'";
	...
	<tr>	
		...
		<td> 
			<select name="banco_receptor" class="normal" id="banco_receptor">
 		<option value="<?php echo $nombre_banco_receptor."-".$numero_cuenta_receptor;?>" selected="selected"><?php echo $nombre_banco_receptor;?>-<?php echo trim($numero_cuenta_receptor);?></option>
			 	<?
			 	$sql="SELECT b.banc_id, upper(b.banc_nombre) as nombre_banco, c.ctab_numero as cuenta_banco from sai_banco b, sai_ctabanco c where b.banc_id=c.banc_id and c.ctab_estatus=1 and ctab_numero<>'".$numero_cuenta_receptor."'";
			 	$resultado_banco=pg_query($conexion,$sql);
				while ($row_banco=pg_fetch_array($resultado_banco)) {
			    ?>
			     <option value="<?php echo trim($row_banco["nombre_banco"])."-".$row_banco["cuenta_banco"];?>"><?php echo $row_banco["nombre_banco"];?> - <?php echo trim($row_banco["cuenta_banco"]);?></option>
