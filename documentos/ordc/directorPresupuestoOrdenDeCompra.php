...
<script><!--
	...
	function aprobar(){
		if(confirm(pACUTE+'Est'+aACUTE+' seguro que desea aprobar esta Orden de Compra?.')){
...
?>
<form name="formularioOrdenDeCompra" method="post" action="directorPresupuestoOrdenDeCompraAccion.php" id="formularioOrdenDeCompra">
		...
		<td colspan="2">
			<table class="tablaalertas">
				<?php 
					...
					$resultadoProveedor = pg_exec($conexion, $query);
					$totalProveedor = pg_numrows($resultadoProveedor);
				?>
				<tr class="td_gray normalNegrita" id="header2">
					...
					<?php
						...
						if($totalProveedor>0){
							$row = pg_fetch_array($resultadoProveedor, 0);
