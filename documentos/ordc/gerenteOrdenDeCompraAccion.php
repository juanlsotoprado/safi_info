...
if($accion==ACCION_APROBAR_REQUISICION){
	...
	if($numeroFilas>0){
		...
		$copiaOculta = "";
		$nombreCopiaOculta = "";
		$asunto = utf8_decode("Nueva Orden de Compra");
		$message = wordwrap(utf8_decode("Se ha aprobado una nueva Orden de Compra con código ".$idOrdc." procedente de ").$depe_nombre, 70);
	...
	if($error==""){
		...
		header("Location:gerenteOrdenDeCompra.php?msg=".$error."&codigo=".$codigo."&codigoCR=".$codigoCR."&idOrdc=".$idOrdc."&tipoRequ=".$tipoRequ."&pagina=".$pagina."&estado=".$estado."&controlFechas=".$controlFechas."&fechaInicio=".$fechaInicio."&fechaFin=".$fechaFin."&accion=".$accionDetalles."&dependencia=".$dependencia."&rifProveedor=".$rifProveedor."&nombreProveedor=".$nombreProveedor."&idItem=".$idItem."&nombreItem=".$nombreItem."&bandeja=".$bandeja,false);
	}
}
ob_end_flush();
pg_close($conexion);
