...
if($accion==ACCION_APROBAR_REQUISICION){
	...
	if($resultado){
		...
		$sql="UPDATE sai_doc_genera SET wfob_id_ini = ".$wfob_id_ini.", wfca_id = ".$wfca_id.", perf_id_act = '".$perfil."', esta_id = ".$estadoAprobado." WHERE docg_id = '".$idOrdc."'";
		$resultado = pg_exec($conexion ,$sql);
	...
	if($error==""){
		...
		if($resultado){
			...
			if($numeroFilas>0){
				...
				//$asunto = utf8_decode("Devolución de Orden de Compra");
				//$message = utf8_decode("Se ha devuelto una Orden de Compra con código ".$idOrdc." procedente de ").$depe_nombre;
				//$message = wordwrap($message, 70);
				//while($row = pg_fetch_array($resultado)){
				//	$mail = new PHPMailer();
				//	$mail->SetLanguage('es');
				//	$mail->From     = "info@infocentro.gob.ve";
				//	$mail->FromName = utf8_decode("SISTEMA ADMINISTRATIVO DE LA FUNDACION INFOCENTRO");
				//	$mail->Host     =  "150.188.84.43";
				//	$mail->Mailer   =  "smtp";
				//	$mail->AddAddress ($row['empl_email'],$row['solicitante']);
				//	$mail->Subject  =  $asunto;
				//	$mail->Body     = $message;
				//	$mail->isHTML(false);
				//	$mail->Send();
				...
				$nombreCopiaOculta = "";
				$asunto = utf8_decode("Devolución de Orden de Compra");
				$message = wordwrap(utf8_decode("Se ha devuelto una Orden de Compra con código ".$idOrdc." procedente de ").$depe_nombre, 70);
				while($row = pg_fetch_array($resultado)){
					$para = $row['empl_email'];
		...
		header("Location:analistaPresupuestoOrdenDeCompra.php?msg=".$error."&codigo=".$codigo."&codigoCR=".$codigoCR."&idOrdc=".$idOrdc."&tipoRequ=".$tipoRequ."&pagina=".$pagina."&estado=".$estado."&controlFechas=".$controlFechas."&fechaInicio=".$fechaInicio."&fechaFin=".$fechaFin."&accion=".$accionDetalles."&dependencia=".$dependencia."&rifProveedor=".$rifProveedor."&nombreProveedor=".$nombreProveedor."&idItem=".$idItem."&nombreItem=".$nombreItem."&bandeja=".$bandeja,false);
	}
}
ob_end_flush();
pg_close($conexion);
