...
if ($valido)
  {
	$sql= "select * from sai_seleccionar_campo ('sai_sol_pago_retencion','sopg_ret_monto,sopg_por_rete,sopg_monto_base','sopg_id='||'''$codigo'' and impu_id=''ISLR''','',2)  resultado_set (sopg_ret_monto float8,sopg_por_rete float4,sopg_monto_base float8)";
	...
	$sql_insert="select * from sai_insert_comprobante ('".$id_compr."','".$fecha_causado."','".$codigo."','".$sopg_bene_ci_rif."','".$nombre_bene."','ISLR') as resultado";
	$resultado=pg_exec($conexion,$sql_insert) or die (utf8_decode("El número de comprobante ya existe"));

	/*$sql="SELECT max(compr_id) as max_seq FROM sai_comprobante_islr"; 
    ...
    if ($secuencia==1){
     $a=date(Y);
	 $sql_act="update sai_seq_comprobante set comp_num='".$secuencia."', anno='".$a."' where comp_tipo='ISLR'";
    }else{
    	$sql_act="update sai_seq_comprobante set comp_num='".$secuencia."' where comp_tipo='ISLR'";
    }
...
class PDF extends FPDF
{
	...
	{  $alto=4;
       ...
       global $deducible;
		...
		$this->SetX(23.5);
		$this->SetY(50);
		$this->Cell(199,25,utf8_decode('IMPUESTO SOBRE LA RENTA'),0,1,'C');
		//Salto de l�nea
		$this->Ln(1);
		...
		$this->Cell(35,5,utf8_decode("1.Nº: ".$an_o."-".$mes."-".$secuencia),1,0,'L');
		$this->SetXY(-51.5,57);
...
$pdf->SetXY(26.5,$posy);
  ...
  $pdf->Cell(15,$alto,utf8_decode("% RETENCIÓN"),1,0,'C');
  $pdf->Cell(13.8,$alto,"DEDUCIBLE",1,0,'C');  
  $pdf->Cell(20,4,"IMPUESTO RETENIDO",1,0,'C');  
  ...
  $pdf->Cell(15,$alto ,$por_rete_doc,1,0,'R'); 
  $pdf->Cell(13.8,$alto ,$deducible,1,0,'R'); 
  $pdf->Cell(20,$alto ,$rete_monto_doc,1,0,'R'); 
 ...
 for ($i=0; $i<9;$i++)
 {
  ...
  $pdf->Cell(20.4,$alto ,"",1,0,'R'); 
  $pdf->Cell(20.4,$alto ,"",1,0,'R'); 
  $pdf->Cell(15,$alto ,"",1,0,'R'); 
  $pdf->Cell(13.8,$alto ,"",1,0,'R');
  $pdf->Cell(20,$alto ,"",1,0,'R'); 
...
limpiarTemporalesPdf(dirname(__FILE__));
	...
	<br>
	<div align="center">La solicitud de pago ingresada no tiene Retenci&oacute;n del Impuesto Sobre la Renta</div>
	<br>
