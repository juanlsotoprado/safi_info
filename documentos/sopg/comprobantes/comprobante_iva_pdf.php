...
class PDF extends FPDF
{
	...
	{  $alto=4;
	   ...
	   global $comprobante;
		...
		$this->Cell(30,5,utf8_decode("1.Nº: ".$comprobante),1,0,'L');
		$this->SetXY(-77,57);
...
// $pdf=new PDF('P','mm','letter');
  ...
  $pdf->SetFont('Arial','B',7);
  $pdf->Cell(111.4,8,utf8_decode("AÑO ".$an_o." MES ".$mes),0,2,'L'); 
  $pdf->SetFont('Arial','B',5);
  $posy= $pdf->gety();
  $pdf->SetXY(26.5,($posy-3)); 
  ...
  $pdf->SetFont('Arial','B',7);
  if ($rif==""){
  $pdf->Cell(111.4,8,html_entity_decode(htmlentities($nombre_bene)),0,2,'l'); 
  }else{
  $pdf->Cell(111.4,8,html_entity_decode(htmlentities($proveedor)),0,2,'l');
  }
  $pdf->SetFont('Arial','B',5);
  $pdf->SetXY(137.9,$posy); 
  $pdf->SetFont('Arial','B',5);
  ...
  $pdf->SetFont('Arial','B',7);
  if ($rif==""){
  $pdf->Cell(111.4,8,$sopg_bene_ci_rif,0,2,'L'); 
  }else{
  $pdf->Cell(111.4,8,$rif,0,2,'L'); 
}
