...
if ($valido_ltf=="si")
  {
	...
	$sql_insert="select * from sai_insert_comprobante ('".$id_compr."','".$fecha_causado."','".$codigo."','".$sopg_bene_ci_rif."','".$nombre_bene."','LTF') as resultado";

 	 $resultado=pg_exec($conexion,$sql_insert) or die (utf8_decode("El número de comprobante ya existe"));

	/*$sql="SELECT max(compr_id) as max_seq FROM sai_comprobante_ltf"; 
	$resultado=pg_exec($conexion,$sql);
	 ...
	 $max_seq=(int)substr(trim($rowf['max_seq']),9,18);
 	  ...
 	  if ($secuencia==1){
       $a=date(Y);
  	   $sql_act="update sai_seq_comprobante set comp_num='".$secuencia."', anno='".$a."'  where comp_tipo='LTF'";
 	  }else{
 	  	$sql_act="update sai_seq_comprobante set comp_num='".$secuencia."' where comp_tipo='LTF'";
 	  }
	 $resultado=pg_exec($conexion,$sql_act) or die ("Error al actualizar el comprobante");
