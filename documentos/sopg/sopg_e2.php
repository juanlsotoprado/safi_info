<?php 
	...
	if (trim($_POST['chk_factura'])=="on") $anexos[0]=1; //Factura
	if (trim($_POST['chk_ordc'])=="on")	$anexos[1]=1; //Orden de Compra
	if (trim($_POST['chk_contrato'])=="on") $anexos[2]=1; //Contrato
	if (trim($_POST['chk_certificacion'])=="on") $anexos[3]=1; //Certidicacion del control perceptivo
	if (trim($_POST['chk_recibo'])=="on") $anexos[4]=1; //Recibo
	if (trim($_POST['chk_ords'])=="on") $anexos[5]=1; //Orden de Servicio
	if (trim($_POST['chk_pcta'])=="on") $anexos[6]=1; //Punto de Cuenta	
	if (trim($_POST['chk_gaceta'])=="on") $anexos[7]=1; //Gaceta Oficial	
	if (trim($_POST['chk_informe'])=="on") $anexos[8]=1; //Informe o solicitud de pago a cuentas		
	if (trim($_POST['chk_estimacion'])=="on") $anexos[9]=1; //estimacion o calculo segun RAC	
	if (trim($_POST['chk_otro'])=="on") $anexos[10]=1; //Otros(especifique)			
	...
	/*Fin factura*/
	if ($factura_fecha != '')	$factura_fecha=cambia_ing($factura_fecha);
	$prioridad=trim($_POST['slc_prioridad']);
	
	/*Fuente de financiamiento --BORRAR*/
  	if(trim($_POST['hid_otro'])==1) {
		$sopg_sustitucion=trim($_POST['hid_susti_fuente']);
		$fuente=trim($_POST['hid_susti_fuente']);
  	}
  	else $sopg_sustitucion=trim($_POST['slc_sustitucion']);
 	/* Fin Fuente de financiamiento --BORRAR */ 
	...
	if ($monto_iva_impu >0) { //Ojo: aquí está el problema; si el IVA proviene de diferentes Proyectos; pej: Caso Caja Chica
		for($j=0; $j<$total_imputacion; $j++) {
	  		$valor=$partida_IVA;
	  		$clave = array_search($valor, $matriz_sub_esp);
	 		if (!is_numeric($clave)) { //OJO: aquí se usa partida del iva del año anterior
	...
	if ($_POST['hid_imputa_tipo']==1) /*Centros por Proyecto*/ 
	...
	else /*Centro por Accion Centralizada*/
	...
	/*Fin imputaciones presupuestarias*/	

	/*Verificación de convertidor y llenado de partidas*/
	...
	/*Actualización de la disponibilidad en sai_disp_comp --Borrar*/	
	if ($_POST['comp_id']!='N/A') { //SI TIENE COMPROMISO ASOCIADO, SE ACTUALIZA LA DISPONIBILIDAD DEL COMPROMISO
	  //PRIMERO SE LIBERA LAS PARTIDAS DEL SOPG ANTES DE LA MODIFICACION,XQ AL MODIFICAR PUEDEN SER MENOS PARTIDAS Y A SU VEZ DIFERENTES
	   ...
	   //SEGUNDO SE VALIDA LA DISPONIBILIDAD DE LAS NUEVAS PARTIDAS DEL SOPG DESPUES DE LA MODIFICACION
		...
		for($k=0; $k<$largo_partidas; $k++) {
	     	...
	     	?>
				  	<script>
					  document.location.href = "mensaje.php?pag=principal.php&msj=LA PARTIDA +<?php echo $part_id;?>+ NO SE ENCUENTRA DISPONIBLE EN EL COMPROMISO";
					</script>
	     	<?php 
	     		}
	     	}
	...
	if ($satisfactorio) { 
	  	/*Temporal - Se debe eliminar este mensaje
	  	echo "Modificada satisfactoriamente la ".$codigo.". Se encuentra actualmente en la bandeja del Coordinador de Ord. de pagos";
	  	$sql = "UPDATE sai_doc_genera 
				SET wfob_id_ini = 3,
				wfca_id = 5,
				esta_id = 10,
				perf_id_act = '42450'
				WHERE docg_id='".$codigo."'";
	  	$resultado_modif = pg_query($conexion,$sql) or die("Error al actualizar doc_genera la solicitud".$sql);*/
				...
				$resultado_anexo = pg_exec($conexion ,$sql_anexo);
			 	...
			 	if($valido && $disponibilidad) {//Si el documento viene de cero.. se registra el iva por documento
 					...
 					$monto_iva="{".str_replace(",","",$_POST['txt_monto_iva_tt'])."}";
 				 	...
 				 	/*Borrado IVA anterior - Cálculo IVA nuevo */
		...
		} //endif($resultado_modif) //VOLVER A DESCOMENTAR MIENTRAS SE CAMBIO DE CONDICION
		$docu_base=  $codigo;
		$elem_imp_iva=0;
 ...
 <?php
	...
	";
		...
		if (($resultado = pg_query($conexion, $query))===false)	{
			error_log("Error al consultar las retenciones del sopg = '".trim($codigo)."'");
...
?>		
        ...
        <td height="21" ><input name="chk_factura" type="checkbox" id="chk_factura"  disabled <?php if ($anexos[0]==1){echo "checked";}?> /></td>
		 <td height="21">Factura</td>
        <td height="21" ><input name="chk_ordc" type="checkbox" id="chk_ordc" disabled <?php if ($anexos[1]==1){echo "checked";}?>/></td>
		<td height="21" >Orden de Compra</td>
        <td height="21" ><input name="chk_contrato" type="checkbox" id="chk_contrato" disabled <?php if ($anexos[2]==1){echo "checked";}?>/></td>
		<td height="21" >Contrato</td>
        <td height="21" ><input name="chk_certificacion" type="checkbox" id="chk_certificacion" disabled <?php if ($anexos[3]==1){echo "checked";}?>/></td>
        <td height="21" >Certificaci&oacute;n del Control Perceptivo</td>
        </tr>
		
		<tr  class="normalNegro"> 
        <td height="21" ><input name="chk_informe" type="checkbox" id="chk_informe" disabled <?php if ($anexos[8]==1){echo "checked";}?>/></td>
		<td height="21">Informe o Solicitud de Pago a Cuentas</td>
        <td height="21" ><input name="chk_ords" type="checkbox" id="chk_ords" disabled <?php if ($anexos[5]==1){echo "checked";}?>/></td>
        <td height="21" >Orden de Servicio</td>
        <td height="21" ><input name="chk_pcta" type="checkbox" id="chk_pcta" disabled <?php if ($anexos[6]==1){echo "checked";}?>/></td>
        <td height="21" >Punto de Cuenta</td>
	    </tr>

		<tr class="normalNegro"> 
        <td height="21" ><input name="chk_otro" type="checkbox" id="chk_otro" disabled <?php if ($anexos[10]==1){echo "checked";}?>/></td>
        <td height="21" >Otro (Especifique)</td>
		<td height="21"  ></td>
        <td height="21" ><input type="text" name="txt_otro" id="txt_otro"  size="25" maxlength="25" value="<?php echo $anexos_otros;?>"   disabled></td>
        </tr>
			...
			<tr>
				...
				<tr class="normal"> 
				<td height="28" colspan="2"> <div class="normalNegrita">Factura N&deg;:
				<input name="txt_factura"  type="text" class="normalNegro" id="txt_factura" size="20" maxlength="20"   align="right"  readonly value="<?php echo $factura_num;?>"/>
				  Fecha:
				  <input name="txt_fecha_factura" type="text" class="normalNegro" id="txt_fecha_factura" size="12" readonly  value="<?php echo $factura_fecha;?>" />
				  N&deg; de control:
				  <input name="txt_factura_num_control"  type="text" class="normalNegro" id="txt_factura_num_control" size="11" maxlength="10"    align="right"  readonly  value="<?php echo $factura_control;?>"/>
				  </div></td>
				 </tr>
	  		<?php } ?>
		...
		<select name="slc_prioridad" class ="normalNegro" disabled >
          ...
          <tr class="normal"> 
		   <td height="28" class="normalNegrita">Fuente de financiamiento:</td>
		   <td class="normalNegro"><?echo $numero_reserva?></td>
		   </tr>
		  
		  	<tr class="normal"><td height="28"  class="normalNegrita">N&uacute;mero del compromiso:</td>
	  ...
	  <td colspan="2">
		...
		?> 		 <tr>
                  <td  class="peq" align="left" width="17%">
                    <div align="center">
                      <input name=<?php echo "txt_imputa_proyecto_accion".$ii;?>  type="text" class="normalNegro" id=<?php echo "txt_imputa_proyecto_accion".$ii;?> size="15" maxlength="15"    align="right" value="<?php echo  $cg;?>" readonly/>
                    </div></td>
                  <td  class="peq" align="left" width="19%">
                    <div align="center">
                      <input name=<?php echo "txt_imputa_accion_esp".$ii;?>  type="text" class="normalNegro" id=<?php echo "txt_imputa_accion_esp".$ii;?> size="15" maxlength="15"  align="right" value="<?php echo $cc;?>" readonly/>
                    </div></td>
                  <td  class="peq" align="left" width="10%">
                    <div align="center">
                      <input name=<?php echo "txt_imputa_unidad".$ii;?>  type="text" class="normalNegro" id=<?php echo "txt_imputa_unidad".$ii;?> size="10" maxlength="10"  align="right" value="<?php echo $matriz_uel[$ii];?>" readonly/>
                    </div></td>
                   ...
                   <?php 	
                    if (substr($matriz_sub_esp[$ii],0,6)=="4.11.0"){
     				...
     				$cuenta=$matriz_sub_esp[$ii];
                      ...
                      <input name="<?php echo "txt_imputa_sub_esp".$ii;?>"  type="text" class="normalNegro" id="<?php echo "txt_imputa_sub_esp".$ii;?>" size="15" maxlength="15"  align="right" value="<?php echo $cuenta;?>" readonly title="<?php if (trim( $matriz_sub_esp[$ii])== $partida_IVA){echo "Impuesto al Valor Agregado";}?>"/>
                    </div></td>
                  <td  class="peq" align="left" width="39%">
                    <div align="center">
                      <input name="<?php echo "txt_imputa_monto".$ii;?>"  type="text" class="normalNegro" id="<?php echo "txt_imputa_monto".$ii;?>" size="25" maxlength="25" align="right" value="<?php echo  number_format($matriz_monto[$ii],2,'.',',');?>" readonly />
                    </div></td>
		    <td  class="peq" align="left" width="39%">
                    <div align="center">
                      <input name="<?php echo "txt_imputa_monto_exento".$ii;?>"  type="text" class="normalNegro" id="<?php echo "txt_imputa_monto_exento".$ii;?>" size="25" maxlength="25"  align="right" value="<?php echo  number_format($matriz_monto_exento[$ii],2,'.',',');?>" readonly />
                    </div></td>
                </tr>
                <?php 
		 }
		...
		<td   align="center" class="normal"  colspan="2">
			...
			<tr class="td_gray"> 
		<td height="21" colspan="2"><div align="left" class="normalNegroNegrita"> 
		...
		En Bs.
		    <input type="text" name="txt_monto_tot" value="<?php echo trim($_POST['txt_monto_tot']); ?>" size="20" readonly class="normalNegro">
		</span></div>		</td>
		</tr>
		<tr>
		<td height="31" colspan="2">
		...
		En Letras:<textarea name="txt_monto_letras" id="txt_monto_letras" rows="2" cols="70" class="normalNegro" readonly><?php echo trim($_POST['txt_monto_letras']);?></textarea>
			...
			<tr>
				<td height="117" colspan="2" class="normal"><br>
					<div align="center">
					Esta solicitud de pago fue modificada el d&iacute;a: 
  	...
  	?>
 		<table width="500" align="center" background="imagenes/fondo_tabla.gif" class="tablaalertas">
  		<tr class="td_gray"> 
   		<td height="21" colspan="3"><div align="left" class="normalNegrita"> 
