...
if($row["perf_id_act"]!='15456' && $row["perf_id_act"]!='42456'){
...
<head>
	<title>.:SAFI:Requisici&oacute;n de Compra</title>
	...
	<script language="Javascript">
		...
		function verProveedoresTipo(elemento, tipoProv){
			if(elemento.checked==true){				
				idRequ = '<?= $idRequ?>';
				tipoRequ = '<?= $tipoRequ?>';
				parameters = "idRequ="+idRequ+"&tipoRequ="+tipoRequ+"&tipoProv="+tipoProv;
				xmlhttp = ajaxFunction();
				xmlhttp.onreadystatechange=function(){
					if(xmlhttp.readyState==4){
						if(tipoProv=="<?= TIPO_PROVEEDORES_TODO?>"){
							document.getElementById("divProveedoresTodo").innerHTML = xmlhttp.responseText;
						}else if(tipoProv=="<?= TIPO_PROVEEDORES_UNO?>"){
							document.getElementById("divProveedoresUno").innerHTML = xmlhttp.responseText;
						}else if(tipoProv=="<?= TIPO_PROVEEDORES_MATRIZ?>"){
							document.getElementById("divProveedoresMatriz").innerHTML = xmlhttp.responseText;
						}
						document.getElementById("divAcciones").style.display = "block";
		  			}
		  		};
				xmlhttp.open("POST","proveedores.php",true);
				...
				if(tipoProv=="<?= TIPO_PROVEEDORES_TODO?>"){
					document.getElementById("divProveedoresTodo").innerHTML = "";
				}else if(tipoProv=="<?= TIPO_PROVEEDORES_UNO?>"){
					document.getElementById("divProveedoresUno").innerHTML = "";
				}else if(tipoProv=="<?= TIPO_PROVEEDORES_MATRIZ?>"){
					document.getElementById("divProveedoresMatriz").innerHTML = "";
				}
				if(document.getElementById("divProveedoresTodo").innerHTML==""
					&& document.getElementById("divProveedoresUno").innerHTML==""
					&& document.getElementById("divProveedoresMatriz").innerHTML==""){
		...
		function crearSolicitudCotizacion(){
			cadenaProveedores = "";
			...
			if(cadenaProveedores!=""){
				if(confirm(pACUTE+'Est'+aACUTE+' seguro que desea crear una solicitud de cotizaci'+oACUTE+'n para esta requisici'+oACUTE+'n?.')){
					...
					document.formularioSolicitudCotizacion.submit();
				}
			}else{
		...
		function devolver(){
			if(confirm(pACUTE+'Est'+aACUTE+' seguro que desea devolver esta requisici'+oACUTE+'n?.')){
				...
				if(memo!=null && memo!="" && memo!="null" && trim(memo)!=""){
					...
					document.getElementById("memo").value = trim(memo);
					document.formularioSolicitudCotizacion.submit();
				}
			}
		}
		
		function verificarCheckboxControl(tipoProveedor){
			inputs = document.getElementsByTagName("input");
			todosMarcados = true;
			for(var i = 0; i < inputs.length; i++) {
				if(inputs[i].getAttribute("type")=="checkbox"
					&& strStartsWith(inputs[i].getAttribute("name"),"proveedores"+tipoProveedor)==true
					&& inputs[i].checked == false){
					todosMarcados = false;
				}
			}
			checkboxControl = document.getElementById("controlProveedores"+tipoProveedor);
			checkboxControl.checked = todosMarcados;
		}

		function marcarTodosNinguno(tipoProveedor){
			checkbox = document.getElementById("controlProveedores"+tipoProveedor);
			inputs = document.getElementsByTagName("input");
			for(var i = 0; i < inputs.length; i++) {
				if(inputs[i].getAttribute("type")=="checkbox"
					&& strStartsWith(inputs[i].getAttribute("name"),"proveedores"+tipoProveedor)==true){
					inputs[i].checked = checkbox.checked;
					agregarQuitarProveedor(inputs[i]);
				}
			}
			verificarCheckboxControl(tipoProveedor);
		}
...
?>
<form action="../soco/solicitudCotizacion.php" method="post" id="formularioSolicitudCotizacion" name="formularioSolicitudCotizacion">
...
<div id="divAcciones" style="text-align: center;">
	<input type="button" class="normalNegro" value="Solicitar cotizaci&oacute;n" onclick="crearSolicitudCotizacion();"/>
	<?php
		$resultado = pg_exec($conexion,	"SELECT COUNT(soco_id) ".
										"FROM sai_sol_coti ssc ".
										"WHERE ".
											"ssc.rebms_id = '".$idRequ."' ");
		$row = pg_fetch_array($resultado, 0);
		if($row[0]==0){
...
<br/>
<table class="tablaalertas" align="center">
	<tr class="td_gray">
		<td colspan="1"><input type="checkbox" id="tipoProveedoresTodo" name="tipoProveedoresTodo" value="<?= TIPO_PROVEEDORES_TODO?>"  checked="checked" onclick="verProveedoresTipo(this, '<?= TIPO_PROVEEDORES_TODO?>');"/>Proveedores de todos los Ramos</td>
		<td colspan="1"><input type="checkbox" id="tipoProveedoresMatriz" name="tipoProveedoresMatriz" value="<?= TIPO_PROVEEDORES_MATRIZ?>" onclick="verProveedoresTipo(this, '<?= TIPO_PROVEEDORES_MATRIZ?>');"/>Matriz de Proveedores y Ramos</td>
		<td colspan="1"><input type="checkbox" id="tipoProveedoresUno" name="tipoProveedoresUno" value="<?= TIPO_PROVEEDORES_UNO?>" onclick="verProveedoresTipo(this, '<?= TIPO_PROVEEDORES_UNO?>');"/>Proveedores por Ramo</td>
	</tr>
</table>
<div id="divProveedoresTodo">
<?php
	/*if($numeroFilas>0){*/
...
<div id="divProveedoresMatriz"></div>
<div id="divProveedoresUno"></div>
<?php
}
?>
</body>
